...
from data.sort import sort_address_jobs, sort_date_jobs
...
def index():
    ...
    if request.method == "POST":
        if form.submit_address.data and form.my_address.data != '':
            ...
            if form.sorting.data == 'date_sort_up':
                jobs = sort_date_jobs('up')
            elif form.sorting.data == 'date_sort_down':
                jobs = sort_date_jobs('down')
    if jobs is None:
        jobs = session.query(Jobs).all()
    return render_template("index.html", title='Добро начинается с тебя', jobs=jobs, form=form)
...
def add_job():
    ...
    return render_template('jobs.html', title='Добавление обращения',
                           form=form)
...
def about_job(job_id):
    ...
    if request.method == "POST":
        ...
        if form.submit_refuse.data:
            job.employee = 0
    session.commit()
    return render_template("about.html", job=job, employer=employer, form=form, title='Добро начинается с тебя')
